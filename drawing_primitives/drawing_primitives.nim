...
# proc drawRect*(rect: Rect, color: Color, filled = true)
...
# proc drawRoundedRect*(rect: Rect, radius: float32, color: Color, filled = true)
...
proc drawRoundedRect*(rect: Rect, radius: float32, color: Color,
    filled = true) =
...
# proc drawLine*(x1, y1, x2, y2: float32, color: Color, thickness = 1.0f32)
...
# proc drawDashedLine*(x1, y1, x2, y2: float32, dashLen: float32, color: Color)
...
# proc drawArc*(centerX, centerY, radius: float32, startAngle, endAngle: float32, color: Color)
...
proc drawArc*(centerX, centerY, radius: float32,
              startAngle, endAngle: float32, color: Color) =
...
# proc drawPie*(centerX, centerY, radius: float32, startAngle, endAngle: float32, color: Color)
...
# proc drawBezier*(points: seq[Point], color: Color, thickness = 1.0f32) (line 345)
...
proc drawCheckmark*(rect: Rect, color: Color, thickness = 2.0f32) =
  ## Draws a checkmark
...
# proc drawRadioCircle*(rect: Rect, selected: bool, color: Color) (line 394)
...
proc drawRadioCircle*(rect: Rect, selected: bool, color: Color) =
...
# proc drawRadioCircleWithUnicode(rect: Rect, selected: bool, color: Color) =
...
# proc drawFocusRing*(rect: Rect, color: Color) (line 408)
...
proc drawFocusRing*(rect: Rect, color: Color) =
...
proc fadeColor(color: Color, factor: float32): Color =
  return Color(
...
proc drawResizeHandle*(rect: Rect, color: Color) =
...
proc drawBadge*(text: string, rect: Rect, color: Color,
                textColor: Color, style: TextStyle) =
...
type
  ...
  GroupBoxStyle* = object
    ...
    titleBackgroundColor*: Option[Color] # If you want title bg different from box
...
proc drawDivider*(rect: Rect, color: Color, vertical = false) =
...
proc drawValidationMark*(rect: Rect, state: Res,
  ...
  let markColor = if color.isSome: color.get else:
    case state
    of Success: Color(r: 76, g: 175, b: 80, a: 255)         # Material Green
    of Error: Color(r: 244, g: 67, b: 54, a: 255)           # Material Red
    of Warning: Color(r: 255, g: 152, b: 0, a: 255)         # Material Orange
...
proc drawAlertSymbol*(rect: Rect, style: AlertLevel,
  ...
  let symbolColor = if color.isSome: color.get else:
    case style
    of Info: Color(r: 33, g: 150, b: 243, a: 255)            # Material Blue
    of Alert: Color(r: 255, g: 152, b: 0, a: 255)            # Material Orange
    of Critical: Color(r: 244, g: 67, b: 54, a: 255)         # Material Red
...
proc drawBusyIndicator*(rect: Rect, progress: float32, color: Color) =
...
proc drawHighlight*(rect: Rect, color: Color) =
...
proc drawSelectionRect*(rect: Rect, color: Color) =
...
proc drawFocusHighlight*(rect: Rect, color: Color) =
...
proc drawDisabledOverlay*(rect: Rect) =
  ...
  drawRect(rect, Color(r: 128, g: 128, b: 128, a: 128))

# Additional Useful Controls

# proc drawIndeterminateProgress*(rect: Rect, offset: float32, color: Color) (line 367)
...
proc drawIndeterminateProgress*(rect: Rect, offset: float32, color: Color) =
...
# proc drawToggleSwitch*(rect: Rect, isOn: bool, color: Color)
...
proc drawToggleSwitch*(rect: Rect, isOn: bool, color: Color) =
  ...
  drawRoundedRect(
    ...
    radius,
    if isOn: color else: Color(r: 200, g: 200, b: 200, a: 255)
  )
  ...
  drawShadow(
    ...
    Shadow(
      blur: 4,
      spread: 0,
      color: Color(r: 0, g: 0, b: 0, a: 64),
      offsetY: 2
    )
  )

proc drawSlider*(rect: Rect, value: float32, color: Color) =
